-- Per-night sleep-stage composition for the Grafana stage panels.
--
-- One row per (user_id, sleep_day) with minutes and % for light/deep/rem/awake,
-- plus a 7-day rolling mean of each %. sleep_day uses the same cluster + 18:00
-- wake-day cutoff as sleep_rolling_stats, so stage panels line up with the trend.
--
-- Stage codes: 4=Light 5=Deep 6=REM; awake = 1(Awake)+3(OutOfBed)+7(AwakeInBed).
-- % is taken over the sum of those four buckets, so light+deep+rem+awake = 100%
-- (any other/unknown stage codes are ignored). Plain view (recomputed on read);
